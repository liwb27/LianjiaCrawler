...
if not BRIEF_MODE:
    #开始读取详细信息
    (house_list_delete, house_list_error) = get_houselist_detail(house_set, MONGO_CONN, update = IS_UPDATE_DB)
    #记录完成情况
    should_done = len(house_set)
    delete = len(house_list_delete)
    err = len(house_list_error)
    print("任务结束，成功" + str(should_done - delete - err) + "条，消失" + str(delete) + "条，出错" + str(err) + "条。")
    print("详见" + FILE_HOUSE_DELETE_LIST + ',' +  FILE_HOUSE_ERROR_LIST)
    #存储已消失房源列表
    house_list_delete_fromfile = read_list(FILE_HOUSE_DELETE_LIST)
    save_list(list(set(house_list_delete) | set(house_list_delete_fromfile)), FILE_HOUSE_DELETE_LIST)
    #存储错误列表
