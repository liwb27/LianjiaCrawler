...
def get_house_detail(html):
    ...
    # 房源特色
    table_name = bsObj.find("div", {"class": "introContent showbasemore"}).findAll(
        "div", {"class": "name"})
    table_content = bsObj.find("div", {"class": "introContent showbasemore"}).findAll(
        "div", {"class": "content"})
    ...
    house["小区名称"] = table_aroundInfo.find(
    ...
    house["_id"] = int(table_aroundInfo.find(
        "div", {"class": "houseRecord"}).find("span", {"class": "info"}).next)
    # todo：经纪人带看情况
